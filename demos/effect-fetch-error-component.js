...
class EffectFetchErrorDemoComponent extends HTMLElement {
  static get __htmsTemplate() {
    if (!this.__templateCache) {
      const template = document.createElement('template');
      template.innerHTML = '<div class="demo"><button id="load">Load Broken Endpoint</button><p id="status">Status: Idle</p><p id="error" class="error"></p></div>';
        ...
        runtime.registerEffect({
          ...
          run: function () {
            ...
            var execute = async function () {
              try {
                const response = await fetch('https://example.com/demo-error.json', init);
                if (!response.ok) {
        ...
        runtime.registerEffect({
          ...
          run: function () {
            try {
              ...
              const finished = !state.loading && !!state.loadNonce;
              if (finished) {
...
if (!customElements.get('effect-fetch-error-demo')) {
  customElements.define('effect-fetch-error-demo', EffectFetchErrorDemoComponent);
}
export {
  EffectFetchErrorDemoComponent
};